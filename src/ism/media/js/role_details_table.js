...
$(document).ready(function() {
	...
	table = $('#members_table').dataTable( {
		...
		"sAjaxSource": "/roles/" + role_type + "/" + role_id + "/data",
        "sDom": 'lprtip',
        "aoColumns": [
	...
	$("#access_level_box").editable( '/roles/update', {
        "callback": function( sValue, y ) {
        	$("#access_level_box").text(sValue);
        	$("#access_level_box").attr("class", "bold row-" + getAccessColor(parseInt(sValue), colorThresholds));
        },
        "submitdata": function ( value, settings ) {
            return { "id": role_unique_id };
        }
    } );
	
	$("#access_level_box").addClass(
			"row-" + getAccessColor(parseInt($("#access_level_box").text()), colorThresholds)
	);
